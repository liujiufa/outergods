...
import { getBuyDetail, getCardListHorse, mintNftHorse, loginByPass, CastingGetUserInfo, sendDrawAward, sendUserCancelDrawAward , sendUserCancelMint} from '../API'
...
import HorseBanner from '../assets/image/HorseBanner.png'
import Horse1 from '../assets/image/Horse1.png'
import Horse2 from '../assets/image/Horse2.png'
...
export default function Casting(): JSX.Element {
    ...
    useEffect(() => {
        if (id && state.token) {
            getBuyDetail(id).then(res => {
                setBuyInfo(res.data)
                getCardListHorse(res.data.id).then(res => {
                    // console.log(res, "铸造列表")
                    res.data.map((item:any)=>{
                        // eslint-disable-next-line @typescript-eslint/no-unused-expressions
                        item.stringListMap.list = [item.stringListMap.list[0]]
                        return item
                    })
                    // res.data.stringListMap.list = [res.data.stringListMap.list[0]]
    ...
    async function casting(price: number, coinName: string, cardType: number, sellOut: boolean) {
        ...
        dispatch(createSetLodingAction(true))
        mintNftHorse(cardType,4).then(res => {
            ...
            Contracts.example.toMintNft(web3React.account as string, res.data.sign, price).then((res: any) => {
                ...
                sendUserCancelMint(res.data.dataId)
            }).finally(() => {
                dispatch(createSetLodingAction(false))
            })
        }, () => {
    ...
    function rewards(coinName:string) {
        if (web3React.account) {
            sendDrawAward(4,coinName).then((res: any) => {
    ...
    return (
        <div className="Casting">
            <div className="Banner">
                <img src={HorseBanner} alt="" />
            </div>
            <div className="LRlayout">
                <div className="Text"style={{ marginRight: 117 }}>
                    <div className="main">Demon riding ground：</div>
            ...
            <div className="introduceLeft introduce">
                {/* {t('Project Description')}：{BuyInfoD && BuyInfoD.issueIntroduce} */}
                Country: South korea<br></br>
                Address: Jeju-si odeung-dong 394 <br></br>
                Horse ownership can be obtained by owning only one NFT issued. All rides are reserved and closed every Friday.
                Buy this NFT to revitalize and increase the value of the DEMON ecosystem, and take this NFT offline to experience horseback riding and catch two rabbits with a stake in horses!
            </div>
            <div className="CastingList">
                {
                    CardList.map((item, index) => <div className="item" key={index}>
                        <CastingItem data={item} isModel={true} descClass="descHover" ModelClass="ModelHover"></CastingItem>
                        {
