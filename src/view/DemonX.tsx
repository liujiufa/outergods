...
import demonX1 from '../assets/image/demonX1.png'
import demonX2 from '../assets/image/demonX2.png'
...
export default function Casting(): JSX.Element {
    ...
    function invitation() {
        if (!web3React.account) {
            ...
            copy(window.location.origin + window.location.pathname + '#/DemonX?id=1&address=' + web3React.account)
            dispatch(createAddMessageAction(t('Copied')))
        }
    }
    function rewards(coinName:string) {
        if (web3React.account) {
            sendDrawAward(1,coinName).then((res: any) => {
                // console.log(res, "领取发射台奖励加密")
                if (res.code === 200) {
                    dispatch(createSetLodingAction(true))
                    Contracts.example.getAward(web3React.account as string, res.data).then((res: any) => {
        ...
        <div className="Casting">
            ...
            <div className="LRlayout">
                ...
                <div className="Text Text1" style={{ marginLeft: 117 }}>
                    <div className="main">{t('The card is introduced')}</div>
                    <div className="sub">{BuyInfoD && BuyInfoD.cardSystem}</div>
                </div>
            </div>
            <div className="introduceTitle">{t('Issue is introduced')}</div>
            ...
            <div className="CastingList">
                {
                    CardList.map((item, index) => <div className="item" key={index}>
                        <CastingItem isModel={true} data={item}></CastingItem>
                        {
            ...
            <div className="invitation">
                ...
                <div className="invitationLink">
                    <div className="LinkBox">{window.location.origin + '/#/Casting?address=' + AddrHandle(web3React.account as string)}
